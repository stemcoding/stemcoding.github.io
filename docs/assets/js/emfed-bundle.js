...
// dist/client.js
async function getToots(userURL, accountId, limit, excludeReplies, excludeReblogs) {
  const url = new URL(userURL);
  const userId = accountId ?? await (async () => {
...
async function getPostAndReplyToots(userURL, id, excludeReplies, excludePost) {
  let toots = [];
...
function renderToot(toot) {
  ...
  return html`<li class="toot">
    ...
    ${boost && html` <a class="user boost" href="${boost.user_url}">
    ...
    <div class="body">${safe(import_dompurify.default.sanitize(toot.content))}</div>
...
async function loadToots(element) {
  const el = element;
  const toots = await getToots(el.href, el.dataset.tootAccountId, Number(el.dataset.tootLimit ?? 5), el.dataset.excludeReplies === "true", el.dataset.excludeReblogs === "true");
  const list = document.createElement("ol");
  list.classList.add("toots");
  el.replaceWith(list);
  for (const toot of toots) {
    const html2 = renderToot(toot);
    list.insertAdjacentHTML("beforeend", html2);
  }
}
async function loadTootPostAndReplies(element) {
  const el = element;
  const toots = await getPostAndReplyToots(el.href, String(el.dataset.tootId), el.dataset.excludeReplies === "true", el.dataset.excludePost === "true");
  const replies = document.createElement("ol");
  replies.classList.add("toots");
  el.replaceWith(replies);
  for (const toot of toots) {
    const html2 = renderToot(toot);
    replies.insertAdjacentHTML("beforeend", html2);
  }
}
function loadAll() {
