...
function initCustomMenu() {
    ...
    d3.select("#customNodes").text("Nodes: " + parseInt(Cookies.get('customNodes')))
    d3.select("#customDegree").text("Neighbors: " + parseInt(Cookies.get('customNeighbors')) + "ea.")
    d3.select("#customVaccines").text("Vaccines: " + parseInt(Cookies.get('customVaccines')))
    d3.select("#customRefusers").text("Refusers: " + parseInt(Cookies.get('customRefusers')))
    d3.select("#customOutbreaks").text("Outbreaks: " + parseInt(Cookies.get('customOutbreaks')))
...
var maxVax = parseInt(Cookies.get('customNodes'))
$(function() {
    $( "#nodeSlider").slider({
        range: "min",
        min: 10,
        ...
        slide: function (event, ui) {
            //$.cookie.json = false;
            Cookies.set('customNodes', ui.value)
            //$.cookie.json = true;
    ...
    $( "#nodeSlider" ).slider( "value", parseInt(Cookies.get('customNodes')));
});
...
$(function() {
    $( "#degreeSlider").slider({
        ...
        slide: function (event, ui) {
            //$.cookie.json = false;
            Cookies.set('customNeighbors', ui.value)
            //$.cookie.json = true;
            $("#customDegree").text("Neighbors: " + ui.value + "ea.");
            customNeighborChoice = ui.value;
        }
    });
    $( "#degreeSlider").slider( "value", parseInt(Cookies.get('customNeighbors')));
});
...
$(function() {
    $( "#vaccineSlider").slider({
        ...
        slide: function (event, ui) {
            //$.cookie.json = false;
            Cookies.set('customVaccines', ui.value)
            //$.cookie.json = true;
    ...
    $( "#vaccineSlider").slider( "value", parseInt(Cookies.get('customVaccines')))
...
$(function() {
    $( "#outbreakSlider").slider({
        ...
        slide: function (event, ui) {
            //$.cookie.json = false;
            Cookies.set('customOutbreaks', ui.value)
            //$.cookie.json = true;
            $("#customOutbreaks").text("Outbreaks: " + ui.value);
            customOutbreakChoice = ui.value;
        }
    });
    $( "#outbreakSlider").slider( "value", parseInt(Cookies.get('customOutbreaks')))
});
...
$(function() {
    $( "#refuserSlider").slider({
        ...
        slide: function (event, ui) {
            //$.cookie.json = false;
            Cookies.set('customRefusers', ui.value)
            //$.cookie.json = true;
            $("#customRefusers").text("Refusers: " + ui.value);
            customRefuserChoice = ui.value;
        }
    });
    $( "#refuserSlider").slider( "value", parseInt(Cookies.get('customRefusers')))
});
...
function readCookiesJSON() {
    ...
    if (Cookies.get('vaxCookie') == undefined) {
        initCookiesJSON();
    }
    ...
    if (isNaN(customRefuserChoice)) {
        ...
        Cookies.set('customRefusers', 0.05)
...
function initCookiesJSON() {
    /* The following will always call clearCookies(), so it can be removed
    var oldCookieTest = Cookies.get('vaxEasyCompletion');
    ...
    */
    ...
    Cookies.set('vaxCookie', JSON.stringify(cookie), { expires: 365, path: '/' })
}
