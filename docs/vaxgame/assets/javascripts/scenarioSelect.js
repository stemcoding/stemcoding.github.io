...
function createUnlocksCookie() {
    ...
    Cookies.set('vaxUnlocks', stringifiedUnlocks, { expires: 365, path: '/' })
    unlocks = JSON.parse(Cookies.get('vaxUnlocks'))
    console.log(Cookies.get('vaxUnlocks'))
...
function checkScoresCookie() {
    //$.cookie.json = true;
    scores = JSON.parse(Cookies.get('vaxScores'))
...
function createScoresCookie() {
    ...
    Cookies.set('vaxScores', stringifiedScores, { expires: 365, path: '/' })
}

function checkSavesCookie() {
    saves = Cookies.get('vaxSaves')
    if (saves == undefined) {
        var initSaves = 0;
        Cookies.get('vaxSaves', initSaves, { expires: 365, path: '/' })
    }
...
function setDifficultyConstants(scenarioTitle, difficulty) {
    ...
    Cookies.set('vaxCurrentGame', JSON.stringify(currentGameCookie), { expires: 365, path: '/' })
    console.log(Cookies.get('vaxCurrentGame'))
...
function verifyUnlock(selectedLock) {
    ...
    saves = JSON.parse(Cookies.get('vaxSaves'));
    unlocks = JSON.parse(Cookies.get('vaxUnlocks'));
    ...
    d3.select(".verifyUnlockBox").append("text")
        ...
        .on("click", function() {
            ...
            Cookies.remove('vaxSaves')
            Cookies.remove('vaxUnlocks')

            Cookies.set('vaxSaves', saves, { expires: 365, path: '/' })
            Cookies.set('vaxUnlocks', unlocks, { expires: 365, path: '/' })
