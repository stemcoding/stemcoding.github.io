...
* Take the result of adding these numbers together and take the square root
* Take the result of the square root and divide by the number of measurements 
...
If you do the procedure in the previous section and take the measurement minus the average and square it and do that for each number and add it all up you would get 72.9. Then take the square root of that number which gives you 8.53. Then divide that by the number of measurements which is 10 so then you have 0.85 feet as the error on the mean.

So the average distance would be 37.9 feet +/- 0.85 feet, which seems resonable given that the range of values is from 34 feet to 42 feet and that we only did 10 trials. Specifically the uncertainty of 0.85 feet is the uncertainty "on the mean"
...
<b> Verify that the average distance</b> is 44.4 feet with an uncertainty on the mean of +/- 1.4 feet. After you can confirm this with a calculator (or maybe [desmos](http://desmos.com)) confirm it using a spreadsheet!
...
## Up next: [Error bars and animal sizes](errorbar3.html)