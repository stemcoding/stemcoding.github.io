...
function endScenario() {
    gameIsOver = true;
    console.log("end")
    Cookies.remove('vaxCurrentGame', { path: '/' });
...
//    var currentScenarioScoresCookie = {scenario: scenarioTitle, difficulty: difficulty, speedMode:speed, quarantined: quarantinedIndividuals, saved: savedIndividuals};
    ...
    var saves = parseInt(Cookies.get('vaxSaves'));
    saves += savedIndividuals;
    Cookies.set('vaxSaves', saves, { expires: 365, path: '/' })

    Cookies.set('vaxCurrentScenarioScores', JSON.stringify(currentScenarioScoresCookie), { expires: 365, path: '/' })
